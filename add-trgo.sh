...
trgo="$(cat ~/log-install.txt | grep -w "Trojan GO TLS" | cut -d: -f2|sed 's/ //g')"
...
link="trojan-go://${uuid}@bug.com:${trgo}/?sni=${domain}&type=ws&host=${domain}&path=%2Ftrojango#$user"
link1="trojan://${uuid}@bug.com:${trgo}/?sni=${domain}&type=ws&host=${domain}&path=%2Ftrojango#$user"
clear
